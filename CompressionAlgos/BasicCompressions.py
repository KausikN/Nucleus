'''
Summary
Basic Compression Algorithms for generic data
 - Runner Length
'''
...
def RunnerLength_Decompress(filepath=None, data=None, countFormat='dec'):
    decompressed_bytes = ''
    if not filepath == None:
        data_bytes = utils.GetBytesFromFile(filepath).decode('utf8')
...
# Driver Code
data = "AAAAAAAAAAAAAAAAAAAAAAAAABBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBC"
print(len(bytearray(data, encoding='utf8')), bytearray(data, encoding='utf8'))
compdata = RunnerLength_Compress(data=data, groupLength=1)
print(len(compdata), compdata)
decompdata = RunnerLength_Decompress(data=compdata)
